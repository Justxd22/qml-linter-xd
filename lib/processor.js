import { parseFile } from '@oltodo/qml-parser';
...
function extractJsFromQml(filename) {
    // Step 1: Parse QML into an AST
    const qmlAst = parseFile(filename);
    ...
    function traverse(node) {
        ...
        if (code){
			...
			jsBlocks.push({code, location, type});
...
function preprocess(sourceText, filename) {
	...
	const jsBlocks = extractJsFromQml(filename);
		...
		return group.map((err) => {
			...
			if (err.fix && block) {
				...
				adjustedError.fix.range = [
					err.fix.range[0] + block.location.start.offset,
					err.fix.range[1] + block.location.start.offset,
